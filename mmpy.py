...
class MarketMakingStrategy:
    def __init__(self, config, params, gamma=0.1):
        ...
        self.model.load_model(os.path.join(self.params["folder_path"], self.params["model"])) if self.params["model"] != "mm-core.json" else None
    ...
    def compute_alpha_signal(self, state, mid, fair, skew, features):

        if self.params["model"] == "mm-core.json":
            ...
            expected_return = self.model.predict([x])[0] # Important: XGBoost expects 2D input.

            vol = state.get_vol()
            
            k0 = 0.1 # tune this later
            k = k0 / (vol + 1e-6)

            reservation = features["microprice"] + k * expected_return #Your current design: “Fair value = microprice”
    ...
    def detect_regime(self, features):
        ...
        regime_id = self.regime_model.predict(features)[0]

        regime = REGIME_MAP[regime_id]
        ...
        if regime == "LOW_VOL":
            policy["k0_multiplier"] = 1.5
            policy["spread_multiplier"] = 1.0
            policy["alpha_multiplier"] = 1.0

        elif regime == "HIGH_VOL":
            policy["k0_multiplier"] = 0.5
            policy["spread_multiplier"] = 2.0
            policy["alpha_multiplier"] = 0.3

        elif regime == "TRENDING":
            policy["k0_multiplier"] = 0.8
            policy["alpha_multiplier"] = 1.5
            policy["inventory_target"] = np.sign(features["trade_imbalance"])

        elif regime == "TOXIC":
            policy["k0_multiplier"] = 0.3
            policy["spread_multiplier"] = 2.5
            policy["alpha_multiplier"] = 0.0
    ...
    def generate_quotes(self, state): ############## Fair Value Anchoring (not market anchoring)
        ...
        Model expects future price > current microprice
        Market is predicted to move up

        Interpretation:

        “Buy pressure / upward drift likely”
        """

        # regime_signal = self.detect_regime(features) for regime detection

        alpha_signal = self.compute_alpha_signal(state, mid, fair, skew, features)

        center = alpha_signal["center"]
        ...
        signal = {
            ...
            "fair": fair,
            "skew": skew,
            "reservation": alpha_signal["reservation"],
            "alpha": alpha_signal["alpha"], # to check how to get last_signal since we dont use this anymore
...
class Engine:
    ...
    def render_dashboard(self):
        ...
        title=f"{snapshot["title"]["model"]} | {snapshot["title"]["mode"]} | {snapshot["title"]["exchange"]} | {snapshot["title"]["instrument"]} | pnl={pnl_pct_sign}{pnl_pct:.4f}%"
